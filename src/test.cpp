...
int main(int argc, char *argv[])
{
  ...
  desc.add_options()
    ...
    ("blastout", po::value<bool>(), "use BLAST output for MSA")
    ("base_length", po::value<int>(), "model length for BLAST-output MSA")
  ...
  bool blast_outp = false;
  int base_length = -1;
  if(vm.count("blastout")) {
    blast_outp = vm["blastout"].as<bool>();
    cerr << "Using BLAST output for MSA." << endl;
    if(vm.count("base_length")) {
      base_length = vm["base_length"].as<int>();
    } else {
      cerr << "Must set base_length for BLAST-output MSA!" << endl;
      exit(1);
    }
  }
  ...
  if(blast_outp) hmm = false;
  
  if(vm.count("in")) {
    cerr << "Input file was set to " << vm["in"].as<string>() << endl;
    string infile = vm["in"].as<string>();
    Msa msa(infile,blast_outp);
    msa.set_base_length(base_length);
    
    Model model(msa,hmm,blast_outp);
    //    model.dump_premodel(msa,"hoge.premodel");
